 ...
 *
 * Rôle :
 * - Fournir une navigation persistante entre les écrans clés :
 *   - Accueil (Dashboard)
 *   - Transactions (liste + détail)
 *   - Alertes (gestion des dossiers)
 *   - Démo (simulateur)
 *
 * Données attendues :
 * - La route courante (useLocation) pour styliser l’onglet actif.
 *
 * Comportement UI :
 * - Met en évidence la page active (fond bleu léger + bordure).
 * - Affiche un “profil” opérateur en bas (UI démo, statique).
 *
 * ✅ Produit :
 * - Cette sidebar est pensée comme un layout “outil interne” (style console).
 * - Le bloc “Paramètres” est un placeholder (pas de route associée ici).
 */
const Sidebar = () => {
  ...
  /**
   * navClass(path)
   * - Construit les classes Tailwind du lien de navigation.
   * - L’onglet actif dépend de location.pathname (source de vérité route).
   */
    ...
    <div className="w-64 h-screen bg-[#0F172A] border-r border-slate-800 flex flex-col fixed left-0 top-0 z-50">
      ...
      <div className="mt-auto p-4 border-t border-slate-800">
        ...
        <div className="flex items-center gap-3 px-4 py-2 bg-slate-900 rounded-lg border border-slate-800">
          <div className="w-8 h-8 rounded-full bg-blue-900/50 text-blue-400 border border-blue-800 flex items-center justify-center text-xs font-bold">
            JD
          </div>
 ...
 *
 * Rôle :
 * - Définir le layout global (Sidebar + zone de contenu).
 * - Déclarer les routes de l’application.
 *
 * Choix technique :
 * - HashRouter :
 *   - évite les soucis de routing en hébergement statique (GitHub Pages, S3…).
 *
 * Sortie UI :
 * - Sidebar fixe à gauche (largeur 64).
 * - Contenu scrollable à droite, centré (max-w-6xl).
 */
const App = () => {
  return (
    <HashRouter>
